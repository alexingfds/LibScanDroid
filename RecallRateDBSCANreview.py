...
# Add lib of this app in dictionnary
def listoflibbyApp(database,namelib,nameapp):
    # namelib is a array of all lib in database
    for app in database:
        listlib=[]
...
def slideGroundtruthNew(l):
    groundtruth=[]
    if len(l) %2 ==0:
        for i in range(int(len(l) / 2), len(l)):
            groundtruth.append(l[i])
    else:
...
def slideValidationNew(l):
    if len(l)%2 ==0:
        validation = [l[i] for i in range(int(len(l)/2) )if len(l) >0]
    else:
        validation = [l[i] for i in range(int((len(l)-1) / 2)) if len(l) >0]
...
def append_list_as_row(file_name, list_of_elem,header_csv=['file','app','iscorrect','topk','isCorrectMRR','firstrankIscorrect']):
...
if __name__ == "__main__":
    ...
    for topfile in range(len(topnumber)):
        ...
        for listfilefold in range(len(namesfoldValidANDnamesfoldTraining)):
            ...
            # dictionnary of list of library by app
            dictlibraryinapp ={}
            dictlibrary = {}
            Scoredictlibrary = {}
            ...
            for application in  listallApp:
                ...
                #  donner une score aux  librairies du groundtruth
                ...
                if(common_member(thisValidation,list_topkLib)):
                    thisrank =getrecallrank(dictRecScore, thisValidation)
                    if(thisrank == None):
                        thisrank = 0
                    append_list_as_row('Review1/'+topkfilename,[fold,thisapp,1,thistopk, 1 if thisrank > 0 else 0, thisrank ])
                else:
                    thisrank = getrecallrank(dictRecScore, thisValidation)
                    if (thisrank == None):
                        thisrank = 0
                    append_list_as_row('Review1/'+topkfilename, [fold, thisapp, 0, thistopk, 1 if thisrank > 0 else 0, thisrank ])
                print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++===')
                print(thisapp)
